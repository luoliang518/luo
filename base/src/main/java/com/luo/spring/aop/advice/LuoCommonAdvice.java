...
import com.luo.spring.aop.advice.joinPoint.LuoProceedingJoinPoint;
import com.luo.spring.bean.LuoOrdered;

import java.lang.reflect.InvocationTargetException;
import java.lang.reflect.Method;

public abstract class LuoCommonAdvice implements LuoAdvice , LuoOrdered {
    // 需要增强的方法
    private Method aspectJAdviceMethod;
    // 创建增强代理类的工厂
    private LuoAspectInstanceFactory aspectInstanceFactory;
    ...
    public LuoCommonAdvice(Method aspectJAdviceMethod, LuoAspectInstanceFactory aspectInstanceFactory) {
     ...
     * @return
     */
    protected Object invokeAdviceMethod(LuoProceedingJoinPoint pjp, Throwable ex) throws Throwable {
        // 准备方法参数
        ...
        // 存在 LuoProceedingJoinPoint， @Around 通知的调用
        if (pjp != null) {
            if (parameterCount == 0) {
                throw new IllegalStateException("环绕通知的参数中缺少 LuoProceedingJoinPoint");
            } else {
                args[0] = pjp;
            }
        }
