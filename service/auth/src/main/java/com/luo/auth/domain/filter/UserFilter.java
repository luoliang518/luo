...
public class UserFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        // 获取用户信息
        User user = initUser(tokenHeader, ipAddress);
        // 存入本地线程上下文
        ...
        if (TokenConstant.TOKEN_REFRESH_TIME > user.getTokenSurvivalTime().getTime()) {
            // 判断是否已经异步生成token 已生成则直接放入请求头 未生成则发布续命事件生成后放入缓存
            String newToken;
            if ((newToken = cacheAcl.getNewToken(user.getAccount(), ipAddress))!=null) {
                response.setHeader("Authorization", "Bearer "+newToken);
            } else {
                event.publishEvent(new TokenEvent(this, tokenHeader));
            }
        }
        chain.doFilter(request, response);
    }

    private User initUser(String tokenHeader,String ipAddress) {
        if (tokenHeader == null) {
            throw new ServiceException("用户信息错误或不存在，请重新登录！");
        }
        // todo 校验token与缓存token(新token或老token)是否一致
